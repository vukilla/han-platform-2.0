...
# Runs the golden path with:
# - XGen on the GPU queue + real GVHMR pose extraction (Windows GPU worker)
# - XMimic on the GPU queue + real Isaac Lab teacher PPO (Windows GPU worker)
#
# Requires:
# - Mac control-plane stack running (docker compose)
# - Windows GPU worker running (Celery queue=gpu, via Isaac Sim python)
# - GVHMR bootstrapped on Windows (scripts/windows/bootstrap_gvhmr.ps1)
# - GVHMR checkpoints staged on Windows under external/humanoid-projects/GVHMR/inputs/checkpoints
...
MODE="${MODE:-mocap}" # nep | mocap (for now, teacher PPO is privileged anyway)
...
PROJECT_JSON="$(curl -sS -X POST "$API_URL/projects" \
  -H "Content-Type: application/json" \
  -H "$AUTH_HEADER" \
  -d "{\"name\":\"Smoke Project (real)\",\"description\":\"real gvhmr + isaaclab ppo\"}")"
...
echo "== Start XGen job (GPU queue, GVHMR pose) =="
XGEN_JSON="$(curl -sS -X POST "$API_URL/demos/$DEMO_ID/xgen/run" \
  -H "Content-Type: application/json" \
  -H "$AUTH_HEADER" \
  -d "{\"params_json\":{\"video_uri\":\"$VIDEO_URI\",\"requires_gpu\":true,\"pose_estimator\":\"gvhmr\",\"gvhmr_static_cam\":true,\"clip_count\":3,\"frames\":40,\"nq\":12,\"contact_dim\":4}}")"
...
POSE_NPZ_URI="$(printf '%s' "$JOB_JSON" | json_get 'params_json.pose_smplx_npz_uri')"
if [[ -z "$DATASET_ID" ]]; then
  echo "Failed to locate dataset_id in XGen job params_json" >&2
  exit 1
fi
if [[ -z "$POSE_NPZ_URI" ]]; then
  echo "Failed to locate pose_smplx_npz_uri in XGen job params_json (GVHMR likely not configured)" >&2
  exit 1
fi

echo "== Start XMimic job (GPU queue, Isaac Lab teacher PPO) =="
...
OK (REAL GPU end-to-end)
- XGen job:            $WEB_URL/jobs/$XGEN_JOB_ID
- GVHMR pose artifact: $POSE_NPZ_URI
...
If XGen failed at pose estimation:
- On Windows run:
  powershell -ExecutionPolicy Bypass -File .\\scripts\\windows\\bootstrap_gvhmr.ps1
- Place required checkpoints under:
